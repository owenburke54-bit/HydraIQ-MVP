...
function buildInsightsCache(version: number): InsightsCache {
  const prof = getProfile();
  const weight = prof?.weight_kg ?? 0;
  if (weight <= 0) {
    return { version, points14: [], localHistorySorted: [] };
  }

  const points14: DayPoint[] = lastNDatesNY(14)
    .map((date) => {
  ...
  const localHistorySorted = lastNDatesNY(34)
    .reverse()
    .map((day) => {
...
function getInsightsCache(version: number): InsightsCache {
  if (cachedInsights && cachedInsights.version === version) return cachedInsights;
  cachedInsights = buildInsightsCache(version);
...
export default function InsightsPage() {
  ...
  const [hydrationVersion, setHydrationVersion] = useState(() => getHydraVersion());
  ...
  const insightsCache = useMemo(() => getInsightsCache(hydrationVersion), [hydrationVersion]);
  const points14 = insightsCache.points14;
  const whoopSelected = snapshot?.whoop ?? null;
  ...
  useEffect(() => {
    const onChange = () => setHydrationVersion(getHydraVersion());
    window.addEventListener("hydra:datachange", onChange);
    return () => window.removeEventListener("hydra:datachange", onChange);
  }, []);
  ...
  const dayBreakdown = useMemo(() => {
    if (!snapshot || snapshot.targetMl <= 0) return null;
    if (!snapshot.targetDrivers.length) return null;
    return { lines: snapshot.targetDrivers, total: snapshot.targetMl };
  }, [snapshot]);

  const selectedTotals = useMemo(
    () => ({
      actual: snapshot?.actualMl ?? 0,
      target: snapshot?.targetMl ?? 0,
  ...
  const quick = useMemo(() => {
    ...
    if (whoopSelected?.sleep_hours != null || whoopSelected?.recovery_score != null) {
      const parts: string[] = [];
      if (whoopSelected.sleep_hours != null)
        parts.push(`Sleep ${whoopSelected.sleep_hours.toFixed(1)} h`);
      if (whoopSelected.recovery_score != null)
        parts.push(`Recovery ${Math.round(whoopSelected.recovery_score)}%`);
      messages.push({
  ...
  const localHistorySorted = useMemo(
    () => (tab === "history" ? insightsCache.localHistorySorted : []),
    [insightsCache, tab]
  );
